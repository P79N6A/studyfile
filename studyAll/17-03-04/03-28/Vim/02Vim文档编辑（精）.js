...
三种模式：
    '普通模式'：控制'光标移动'，'删除字符'，'段落复制'(按ESC)
    '编辑模式'：包含'插入'模式和'替换'模式，'新增'文字和'修改'文字
    '命令行模式'/'末行模式'：'保存'字，'离开'vim，以及其它设置

'vim newfile'  '新建'一vim空白文件进行编辑
...
其实vim的功能是非常强大的，相关设置非常多，所以用户可以自己配置适合自己使用习惯的vim操作。
同时vim的相关操作也非常多，只有平时多多练习，忘记了就查笔记的，这样会慢慢的积累，
慢慢的就会成为vim高手。 