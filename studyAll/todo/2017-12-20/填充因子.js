...
延伸：
	'索引最终要的参数'：是'填充因子'。
当'创建'一个'新索引'，或'重建'一个'存在的索引'时，你可以'指定一个填充因子'，它是'在索引创建时'索引里的'数据页被填充的数量'。
...
'页拆分都会发生'，因为'在现存的页上'没有'这些数据的空间'。'很多的页拆分'会'降低sqlserver的性能'。   
	举个例子：假定你刚刚'用缺省的填充因子'新'创建了一个索引'。当'sqlserver创建它时'，它'把索引放在相邻的物理页面'上，
因为'数据能够''顺序的读''所以这样会有最优的i/o访问'。但'当表随着、、、增加和改变时'，发生了'页拆分'。当'页拆分发生时'，
'sqlserver必须在磁盘的某处''分配一个新的页'，这些'新的页'和'最初的物理页''不是连续的'。因此，'访问使用的是随机的i/o'，
而'不是有顺序的i/o'，这样'访问索引页会变得更慢'。
	那么'理想的填充因子'是多少呢？它'依赖于应用程序对sqlserver表的读和写的比率'。首要的原则，按照下面的指导：   
