	在Java编程语言中，'最基本的结构'就是两种，一种是'数组'，一种是'模拟指针(引用)',
'所有的数据结构'都可以'用这两个基本结构构造'，HashMap也一样。
当'程序'试图'将多个key-value''放入HashMap中'时，以如下代码片段为例：
...
m.put("v", "9");
	...
	HashMap '采用'一种所谓的'“Hash 算法”'来'决定每个元素'的'存储位置'。
当'程序执行map.put(String,Obect)方法时'，系统将'调用String的hashCode()方法'
'得到其hashCode值'——'每个Java对象都有hashCode()方法'，都可通过该方法获得它的
hashCode值。'得到'这个对象的'hashCode值之后'，'系统'会'根据'该'hashCode值'来
'决定'该'元素'的'存储位置'。源码如下:
...
public V put(K key, V value) {  
    ...
    for (Entry<K,V> e = table[i]; e != null; e = e.next) {  
        Object k;  
        //判断'当前'确定的'索引位置''是否存在''相同hashcode'和'相同key''的元素'，
//如果'存在'相同的hashcode和相同的key的元素，那么'新值覆盖原来的旧值'，'并返回旧值'。  
        //如果'存在相同的hashcode'，那么他们确定的'索引位置就相同'，
//这时'判断'他们的'key是否相同'，如果'不相同'，这时就是'产生了hash冲突'。  
        //Hash'冲突后'，那么'HashMap的单个bucket里''存储的不是一个Entry'，而'是一个 Entry链'。  
        //'系统只能'必须'按顺序遍历每个Entry'，'直到找到'想'搜索的 Entry为止'——如果'恰好要搜索的Entry'
//'位于该Entry链的最末端'（该Entry是最早放入该bucket中），  
        //那'系统必须循环到最后''才能找到该元素'。  
	...
	上面程序中'用到了'一个'重要的内部接口'：'Map.Entry'，每个Map.Entry其实就'是一个key-value对'。
从上面程序中可以看出：'当系统决定存储'HashMap中的'key-value'对'时'，完全'没有考虑'Entry中的'value'，
仅仅'只是根据key'来'计算''并决定'每个'Entry的存储位置'。这也说明了前面的结论：我们完全'可以把Map' 
集合'中的value当成 key的附属'，当'系统决定了key的存储位置之后'，'value'随之'保存在那里'即可.HashMap程序经过我改造，
我故意的构造出了hash冲突现象，'因为HashMap的初始大小16',但是'我在hashmap里面''放了超过16个元素'，
并且我'屏蔽了它的resize()方法'。'不让它去扩容'。这时'HashMap的底层数组Entry[] table''结构如下': 

图见：'http://www.cnblogs.com/peizhe123/p/5790252.html'

	'Hashmap'里面'的bucket出现'了'单链表的形式'，'散列表'要'解决的一个问题'就是'散列值的冲突问题'，
通常是两种方法：'链表法'和'开放地址法'。链表法就是'将相同hash值'的'对象''组织成一个''链表''放在hash值''对应的槽位'；
'开放地址法'是'通过一个探测算法'，当'某个槽位已经被占据'的情况下'继续查找下一个''可以使用的槽位'。
java.util.'HashMap采用'的'链表法的方式'，'链表是单向链表'。'形成单链表的核心代码如下'：
...
void addEntry(int hash, K key, V value, int bucketIndex) {  
    ...
    上面方法的'代码很简单'，但其中包含了一个设计：'系统'总是'将新添加的Entry对象''放入table数组的bucketIndex'
索引处——'如果bucketIndex索引'处已经'有了一个Entry对象'，那'新添加的Entry对象''指向原有的Entry对象'（产生一个Entry链），
如果'bucketIndex'索引处'没有Entry对象'，也就是上面程序代码的'e变量是 null'，也就是新放入的'Entry对象指向 null'，
也就是'没有产生 Entry 链'。

	HashMap里面'没有出现hash冲突时'，'没有形成单链表时'，'hashmap查找元素很快','get()方法能够直接定位到元素'，
但是出现单链表后，单个bucket 里存储的不是一个Entry，而是一个Entry链，系统只能必须按顺序遍历每个Entry，
直到找到想搜索的Entry为止——如果恰好要搜索的Entry位于该Entry链的最末端（该Entry是最早放入该bucket中），
那系统必须循环到最后才能找到该元素。

	当创建 HashMap 时，有一个默认的负载因子（load factor），其默认值为 0.75，这是时间和空间成本上一种折衷：
增大负载因子可以减少 Hash 表（就是那个 Entry 数组）所占用的内存空间，但会增加查询数据的时间开销，
而查询是最频繁的的操作（HashMap 的 get() 与 put() 方法都要用到查询）；减小负载因子会提高数据查询的性能，
但会增加 Hash 表所占用的内存空间。
