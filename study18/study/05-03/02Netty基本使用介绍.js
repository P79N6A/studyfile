...
1.为什么选择Netty
上一篇文章我们已经了解了Socket通信（IO/NIO/AIO）编程，对于通信模型已经有了一个基本的
认识。其实上一篇文章中，我们学习的仅仅是一个模型，如果想把这些真正的用于实际工作中，
那么还需要不断的完善、扩展和优化。比如经典的TCP读包写包问题，或者是数据接收的大小，
实际的通信处理与应答的处理逻辑等等一些细节问题需要认真的去思考，而这些都需要大量的
时间和经历，以及丰富的经验。所以想学好Socket通信不是件容易事，那么接下来就来学习一
下新的技术Netty，'为什么'会'选择Netty'？因为它'简单'！使用Netty'不必编写''复杂'的'逻辑代码'去'实'
'现通信'，再也'不需要'去'考虑性能'问题，'不需要考虑''编码'问题，'半包读写'等'问题'。强大的Netty已
经帮我们实现好了，我们'只需要使用''即可'。

'Netty''是'最流行的'NIO框架'，它的健壮性、功能、性能、可定制性和可扩展性在同类框架都是
首屈一指的。它已经得到成百上千的商业.商用项目验证，如Hadoop的RPC框架Avro、RocketMQ
以及主流的分布式通信框架Dubbox等等。

2.Netty简介
'Netty是'基于Java NIO client-server的'网络应用框架'，使用Netty'可'以'快速开发''网络应用'，
例如'服务器'和'客户'端'协议'。Netty'提供'了一种'新'的'方式'来'开发网络应用程序'，这种新的方式
使它很'容易使用'和'具有很强'的'扩展性'。Netty的内部实现是很复杂的，但是Netty'提供'了
'简单易用的API'从网络处理代码中'解耦业务逻辑'。Netty是'完全基于NIO实现的'，
所以'整个Netty'都'是异步'的。

网络应用程序通常需要'有较高'的'可扩展性'，无论是'Netty'还是'其他'的基于Java 'Nio'的'框架'，
都'会提供'可'扩展性'的解决'方案'。'Netty'中一个'关键'组成部分'是'它的'异步特性'，本片文章将
讨论'同步（阻塞）'和'异步（非阻塞）'的IO来'说明为什么''使用异步代码''解决扩展性'问题以
及'如何使用异步'。
...
4.Helloworld入门
在学习Netty之前，先来'回顾'一下'NIO'的'通信步骤'：

①'创建ServerSocketChannel'，为其'配置非阻塞模式'。

②'绑定监听'，配置TCP参数，录入backlog大小等。

③'创建'一个'独立'的'IO线程'，用于'轮询''多路复用器'Selector。

④'创建Selector'，'将之前'创建的'ServerSocketChannel注册''到Selector'上，并'设置监听标识位'
SelectionKey.OP_ACCEPT。

⑤'启动IO线程'，在'循环体中执行Selector.select()'方法，'轮询就绪的通道'。

⑥当'轮询到'处于'就绪'状态的'通道'时，需要'进行操作位判断'，如果是'ACCEPT状态'，说明是新的
客户端接入，则'调用accept'方法'接收新的客户端'。

⑦'设置'新接入'客户端'的一些'参数'，如非阻塞，并将其'继续注册到Selector上'，'设置监听标识'
位等。

⑧如果轮询的通道'标识位是READ'，则进行'读取'，'构造Buffer对象''等'。
...
①创建两个NIO线程组，一个专门用于网络事件处理（接受客户端的连接），另一个则进行
网络通信的读写。

②创建一个ServerBootstrap对象，配置Netty的一系列参数，例如接受传出数据的缓存大小等。

③创建一个用于实际处理数据的类ChannelInitializer，进行初始化的准备工作，比如设置
接受传出数据的字符集、格式以及实际处理数据的接口。
