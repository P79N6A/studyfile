...
例如：
	...
	java.class.path  Java的class path '系统类加载器'(AppClassLoader)'可以加载的类'的'路径'
都'保存在系统变量(System.getProperty("java.class.path"))中';
...
如果'想直接从String''得到一个Number型的Object'，'则每个Number'类里面'都有valueOf(String s)''这个静态方法'。如：
...
'File对象可'能'作为参数转换''为文件流进行操作'。 具体操作参照Java IO API。
...
/**
 * 从控制台读取输入的数据, System.in (InputStream) 
 * 先转换至InputStreamReader再用BufferedReader进行读取.
 */
...
/**
 * 将一个可序列化的Java的object以流方式写到一个文件中去.  
 */
...
'过网络传输'（'EJB中进行传递参数'和'返回值的Data Class'），也'可以保存到文件中去'（ObjectOutputStream）。
