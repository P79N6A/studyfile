'yum'（全称为 Yellow dog Updater, Modified）'是'一个在Fedora和RedHat
以及SUSE中的Shell前端'软件包管理器'。基於RPM包管理，能够'从指定的服务器'
自动下'载RPM'包并且'安装'，可以'自动处理依赖性关系'，并且'一次安装所有'
依赖的软体包，无须繁琐地一次次下载、安装。yum'提供了查找'、'安装'、'删除'
某一个、一组甚至全部'软件包的命令'，而且命令简洁而又好记。

yum的命令形式'一般是如下'：'yum [options] [command] [package ...]'
其中的'[options]'是'可选'的，选项包括-h（帮助），'-y（当安装过程提示选择全部为"yes"）'，
-q（不显示安装的过程）等等。'[command]为'所'要进行的操作'，'[package ...]''是操作的对象'。
概括了'部分常用的命令'包括：
'自动搜索'最快镜像插件：   yum install yum-fastestmirror
'安装yum图形窗口'插件：    yum install yumex
'查看'可能'批量安装'的'列表'： yum grouplist
1 '安装'
'yum install' '全部安装'
'yum install package1' '安装指定'的安装'包'package1
...
3 '查找和显示'
'yum info package1' 显示'安装包信息package1'
'yum list' 显示所有'已经安装'和'可以安装的程序包'
'yum list package1' 显示'指定程序包安装情况package1'
...
'清除缓存目录下的软件包'及旧的'headers'


比'如'，要'安装游戏'程序组，首先进行查找：
＃：'yum grouplist'
可以发现，可安装的'游戏程序'包'名字是''”Games and Entertainment“'，这样就可以进行安装：
＃：'yum groupinstall "Games and Entertainment"'
所 '有的游戏程序包'就'自动安装了'。在这里'Games and Entertainment的名字'必'须用双引号'选定，
因为'linux下面遇到空格'会'认为文件名结束了'，因此必须'告诉系统'安装的程序包的
'名字是“Games and Entertainment”'而'不是“Games"'。
此外，'还可以修改配置文件/etc/yum.conf''选择安装源'。可见yum进行配置程序有多方便了吧。
...
yum clean all
-----------------------------------------------------------------------------------------------------------
装了个fedora linux不能用中文输入是一件很棘手的事，连搜解决方案都没法搜。
只能勉强用几个拼音碰碰运气，看Google能不能识别了。而我就遇见了这样的事。
解决方案：
yum install scim* -y
yum 命令详解：
