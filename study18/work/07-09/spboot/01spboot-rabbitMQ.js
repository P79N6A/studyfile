'RabbitMQ' 即'一个消息队列'，主要是'用来实现''应用程序'的'异步和解耦'，同时也'能起到消息缓冲'，'消息分发的作用'。

'消息中间件'在'互联网公司'的'使用'中越来越'多'，刚才还看到新闻'阿里将RocketMQ''捐献给'了'apache'，
当然了今天的主角还是讲RabbitMQ。'消息中间件'最主要的'作用是解耦'，
中间件'最标准的用法'是'生产者''生产消息''传送到队列'，
'消费者'从队列中'拿取消息'并'处理'，
生产者不用关心是谁来消费，消费者不用关心谁在生产消息，从而'达到解耦的目的'。
在分布式的系统中，消息队列也会被用在很多其它的方面，
比如：'分布式事务的支持'，'RPC的调用'等等。

以前一直使用的是ActiveMQ，在实际的生产使用中也出现了一些小问题，
在网络查阅了很多的资料后，决定尝试使用RabbitMQ来替换ActiveMQ，
'RabbitMQ'的'高可用'性、'高性能'、'灵活'性等一些特点吸引了我们，查阅了一些资料整理出此文。

RabbitMQ介绍
'RabbitMQ'是'实现AMQP'（高级消息队列协议）的'消息中间件''的一种'，最初起源于金融系统，
'用于在分布式系统'中'存储转发消息'，在'易用性、扩展性、高可用性'等方面'表现不俗'。

RabbitMQ主要是为了实现系统之间的双向解耦而实现的。当生产者大量产生数据时，
消费者无法快速消费，那么需要一个中间层。保存这个数据。

AMQP，即Advanced Message Queuing Protocol，高级消息队列协议，是应用层协议的一个开放标准，
为面向消息的中间件设计。'消息中间件'主要'用于组件'之间'的解耦'，消息的发送者无需知道消息使用者的存在，
反之亦然。'AMQP'的主要'特征'是'面向消息'、'队列'、'路由'（包括点对点和发布/订阅）、'可靠性'、'安全'。

RabbitMQ是一个开源的AMQP实现，服务器端用Erlang语言编写，支持多种客户端，
如：Python、Ruby、.NET、Java、JMS、C、PHP、ActionScript、XMPP、STOMP等，支持AJAX。
'用于'在'分布式系统'中存'储转发消息'，在易用性、扩展性、高可用性等方面表现不俗。

相关概念
通常我们谈到队列服务, 会有三个概念： '发消息者'、'队列'、'收消息者'，RabbitMQ 在这个基本概念之上,
多做了一层抽象, 在'发消息者'和'队列之间', '加入了交换器' (Exchange). 这样'发消息者'和'队列'就'没有直接联系', 
'转而变成''发消息者'把'消息给交换器', '交换器''根据调度策略'再'把消息再给队列'。



左侧 'P' 代表 '生产者'，也就是往 RabbitMQ '发消息的程序'。
'中间'即'是 RabbitMQ'，其中包括了 交换机 和 队列。
右侧 'C' 代表 '消费者'，也就是往 RabbitMQ '拿消息的程序'。
那么，其中比较重要的概念有 4 个，分别为：'虚拟主机'，'交换机'，'队列'，'和绑定'。

'虚拟主机'：'一个虚拟主机''持有一组交换机'、'队列和绑定'。'为什么需要多个虚拟主机'呢？
很简单，'RabbitMQ当中'，'用户只能在虚拟主机'的'粒度进行权限控制'。
因此，如果需'要禁止A组''访问B组'的交换机/队列/绑定，'必须为A和B''分别创建'一个'虚拟主机'。
'每一个RabbitMQ服务器''都有一个默认'的'虚拟主机'   “/”。
'交换机'：Exchange '用于转发消息'，但是'它不会做存储' ，如果'没有 Queue bind' '到 Exchange 的话'，
'它会直接丢弃'掉 'Producer' '发送'过来'的消息'。 这里有一个比较重要的概念：'路由键' 。
'消息到交换机'的时候，'交互机'会'转发到对应'的'队列中'，那么究竟'转发到哪个队列'，就'要根据该路由键'。
'绑定'：也就是'交换机需要和队列相绑定'，这其中如上图所示，是多对多的关系。
交换机(Exchange)
交换机的功能主要是接收消息并且转发到绑定的队列，交换机不存储消息，在启用ack模式后，
交换机找不到队列会返回错误。交换机有四种类型：Direct, topic, Headers and Fanout

Direct：direct 类型的行为是”先匹配, 再投送”. 即在绑定时设定一个 routing_key, 
...
第一个 X - Q1 就有一个 binding key，名字为 orange； X - Q2 就有 2 个 binding key，
名字为 black 和 green。当消息中的 路由键 和 这个 binding key 对应上的时候，
那么就知道了该消息去到哪一个队列中。

Ps：为什么 X 到 Q2 要有 black，green，2个 binding key呢，一个不就行了吗？ - 这个主要是因为可能又有 Q3，
而Q3只接受 black 的信息，而Q2不仅接受black 的信息，还接受 green 的信息。
...
比如说，一个路由模式是这样子：agreements..b.*，那么就只能匹配路由键是这样子的：第一个单词是 agreements，
第四个单词是 b。 井号（#）就表示相当于一个或者多个单词，例如一个匹配模式是agreements.eu.berlin.#，
...
消息中也包括一组键值对( headers 属性), 当这些键值对有一对, 
或全部匹配时, 消息被投送到对应队列.
...
springboot集成RabbitMQ非常简单，如果只是简单的使用配置非常少，
springboot提供了spring-boot-starter-amqp项目对消息各种支持。
...
发送端加入参数计数，接收端打印接收到的参数，下面是测试代码，
发送一百条消息，来观察两个接收端的执行效果
